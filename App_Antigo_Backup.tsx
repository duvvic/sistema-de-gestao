...
function App() {
  ...
  // REMOVIDO: previousView, navegação será apenas por navigationStack
  ...
  // Nova Tarefa: escuta evento global para abrir tela/modal de cadastro
  useEffect(() => {
    const handleOpenTaskForm = (e: Event) => {
      // Espera que o evento seja CustomEvent<{ clientId, projectId, userId }>
      const detail = (e as CustomEvent<any>).detail || {};
      // Armazena globalmente para TaskDetail
      (window as any).lastTaskFormEventDetail = detail;
      // Seta contexto se vier
  ...
  const handleLogout = () => {
    ...
    // setPreviousView(null); // REMOVIDO: navegação agora é por navigationStack
  };
  ...
  const navigateBack = () => {
    if (navigationStack.length > 0) {
      ...
      // Se o menu ativo for 'clients' e a tela anterior não for do menu de clientes, volta para 'admin'
      if (activeMenu === 'clients' && !['admin', 'client-details', 'client-create'].includes(previousView)) {
        setCurrentView('admin');
      } else if (activeMenu === 'tasks' && previousView !== 'kanban') {
        setCurrentView('kanban');
      } else if (activeMenu === 'team' && previousView !== 'team-list') {
        setCurrentView('team-list');
      } else if (activeMenu === 'dev-projects' && previousView !== 'developer-projects') {
        setCurrentView('developer-projects');
      } else if (activeMenu === 'dev-tasks' && previousView !== 'user-tasks') {
        setCurrentView('user-tasks');
      } else {
        setCurrentView(previousView);
      }
    } else {
      // Fallback: volta para a tela inicial baseado no menu ativo ou role
      if (activeMenu === 'clients') {
        ...
        setCurrentView('team-list');
      } else if (activeMenu === 'dev-projects') {
        setCurrentView('developer-projects');
      } else if (activeMenu === 'dev-tasks') {
        setCurrentView('user-tasks');
  ...
  const handleTimesheetNav = () => {
    if (currentUser?.role === 'admin') {
      ...
      setTimesheetAdminReturnToStatus(true); // Inicia na aba de status
    } else {
  ...
  const handleStatusChange = (taskId: string, newStatus: Task['status']) => {
    ...
    if (task && task.status !== newStatus) {
      ...
      // 1. Move to top locally (Optimistic UI)
      setTasks(prev => {
        const others = prev.filter(t => t.id !== taskId);
        return [updated, ...others];
      });

      // 2. Call save task which handles DB update and may verify rules (e.g. completion modal)
      // Note: handleSaveTask uses prev.map, so it preserves the new order.
        ...
        <div className="absolute inset-0 p-6 lg:p-10 overflow-hidden flex flex-col">
          ...
          {currentView === 'developer-projects' && (
            <DeveloperProjects
              ...
              onNewProject={() => navigateTo('project-create')}
          ...
          {(currentView === 'task-detail' || currentView === 'task-create') && (
            <TaskDetail
              ...
              onBack={() => {
                // Se veio do fluxo de projeto, volta para o detalhe do projeto
          ...
          {currentView === 'timesheet-admin-dashboard' && (
            <TimesheetAdminDashboard
              ...
              onUserTimesheetClick={(userId) => {
                ...
                // setPreviousView('timesheet-admin-dashboard'); // REMOVIDO: navegação agora é por navigationStack
                navigateTo('timesheet-calendar');
              }}
            />
          )}
