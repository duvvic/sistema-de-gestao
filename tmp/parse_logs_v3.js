const fs = require('fs');
try {
    const rawData = fs.readFileSync('C:\\Users\\login\\.gemini\\antigravity\\brain\\cb205bd9-e348-4fe1-8d18-ae0168e97f6f\\.system_generated\\steps\\6809\\output.txt', 'utf8');
    ...
    // Also check for grant_type=password
    console.log('\nChecking for grant_type=password...');
    logs.forEach((l, index) => {
        if (l.event_message.includes('grant_type\":\"password\"')) {
            console.log(`--- Password Grant Found at Index ${index} ---`);
            console.log(l.event_message);
        }
    });
} catch (err) {
